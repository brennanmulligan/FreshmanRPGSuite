 ...
 *
 */
public abstract class ObjectiveStateTableDataGatewayTest extends DatabaseTest
{
	 ...
	 */
	public abstract ObjectiveStateTableDataGateway getGateway();
	...
	{
		gateway = getGateway();
		ArrayList<ObjectiveStateRecordDTO> records = gateway.getPendingObjectivesForPlayer(1);
		...
		{
			if ((record.getObjectiveID() == first.getObjectiveID() && (record.getQuestID() == first.getQuestID())))
			{
					...
					&& (record.getQuestID() == seventh.getQuestID())))
			{
				...
				assertEquals(seventh.isNeedingNotification(), record.isNeedingNotification());
			}
					...
					&& (record.getQuestID() == eighth.getQuestID())))
			{
				...
				assertEquals(eighth.isNeedingNotification(), record.isNeedingNotification());
			}
					...
					&& (record.getQuestID() == ninth.getQuestID())))
			{
				...
				assertEquals(ninth.isNeedingNotification(), record.isNeedingNotification());
			}
			else
			{
	...
	{
		gateway = getGateway();
		ArrayList<ObjectiveStateRecordDTO> records = gateway.getPendingObjectivesForPlayer(3);
	...
	{
		gateway = getGateway();
		ArrayList<ObjectiveStateRecordDTO> records = gateway.getUncompletedObjectivesForPlayer(3);
		assertEquals(0, records.size());
	}
	...
	public void canChangeExisting() throws DatabaseException
	{
		...
		gateway.updateState(adv.getPlayerID(), adv.getQuestID(), adv.getObjectiveID(), ObjectiveStateEnum.COMPLETED,
				false);

		ArrayList<ObjectiveStateRecordDTO> actual = gateway.getObjectiveStates(adv.getPlayerID(), adv.getQuestID());
		int count = 0;
		for (ObjectiveStateRecordDTO asRec : actual)
		{
			if (asRec.getQuestID() == adv.getQuestID() && asRec.getObjectiveID() == adv.getObjectiveID())
			{
				count = count + 1;
