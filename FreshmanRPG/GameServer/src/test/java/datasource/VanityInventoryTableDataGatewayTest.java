 ...
 */
public abstract class VanityInventoryTableDataGatewayTest extends DatabaseTest
{
    protected VanityInventoryTableDataGatewayInterface gateway;

    abstract VanityInventoryTableDataGatewayInterface findGateway() throws DatabaseException;
    ...
    {
        VanityInventoryTableDataGatewayInterface a = findGateway();
        VanityInventoryTableDataGatewayInterface b = findGateway();
        assertEquals(a, b);
        assertNotNull(a);
    }
    ...
    public void getAllOwnedItemsTest() throws DatabaseException
    {
        ...
        int numMatches = 0;
        for (int i = 0; i < mockOwnedItemIDs.size(); i++)
        {
            for(int j = 0; j < itemsFromGateway.size(); j++)
            {
                if(mockOwnedItemIDs.get(i) == itemsFromGateway.get(j).getID())
                {
                    numMatches++;
                }
            }
    ...
    public void getIsWearingTest() throws DatabaseException
    {
        ...
        for (PlayerOwnsVanityForTest p : PlayerOwnsVanityForTest.values())
        {
            if (p.getPlayerID() == PlayersForTest.JOHN.getPlayerID() && p.getIsWearing() == 1)
            {
                items.add(p);
            }
        }
    ...
    {
        VanityDTO item = new VanityDTO(VanityForTest.MerlinShirt.getId(), VanityForTest.MerlinShirt.getName(),
                VanityForTest.MerlinShirt.getDescription(), VanityForTest.MerlinShirt.getTextureName(),
                VanityType.fromInt(VanityForTest.MerlinShirt.getVanityType()));
        ArrayList<VanityDTO> actualItems = gateway.getAllOwnedItems(PlayersForTest.JOHN.getPlayerID());
    ...
    public void canUpdateInventoryMultiple() throws DatabaseException
    {
        ...
        VanityDTO item = new VanityDTO(VanityForTest.MerlinShirt.getId(), VanityForTest.MerlinShirt.getName(),
                VanityForTest.MerlinShirt.getDescription(), VanityForTest.MerlinShirt.getTextureName(),
                VanityType.fromInt(VanityForTest.MerlinShirt.getVanityType()));
        VanityDTO item2 = new VanityDTO(VanityForTest.BlueShirt.getId(), VanityForTest.BlueShirt.getName(),
                VanityForTest.BlueShirt.getDescription(), VanityForTest.BlueShirt.getTextureName(),
    ...
    {
        ArrayList<VanityDTO> actualItemsPrev = gateway.getAllOwnedItems(PlayersForTest.JOHN.getPlayerID());

        gateway.updateInventory(PlayersForTest.JOHN.getPlayerID(), actualItemsPrev);

        ArrayList<VanityDTO> actualItemsNew = gateway.getAllOwnedItems(PlayersForTest.JOHN.getPlayerID());

        assertTrue(actualItemsPrev.containsAll(actualItemsNew) && actualItemsNew.containsAll(actualItemsPrev));
    ...
    {
        ArrayList<VanityDTO> actualItems = gateway.getAllOwnedItems(PlayersForTest.JOHN.getPlayerID());
        ArrayList<VanityDTO> actualWearing = gateway.getWearing(PlayersForTest.JOHN.getPlayerID());
        boolean found = false;
        int i = 0;
        VanityDTO item = null;
    ...
    {
        ArrayList<VanityDTO> actualItems = gateway.getAllOwnedItems(PlayersForTest.JOHN.getPlayerID());
        VanityDTO item = new VanityDTO(VanityForTest.TutorShirt.getId(), VanityForTest.TutorShirt.getName(),
                VanityForTest.TutorShirt.getDescription(), VanityForTest.TutorShirt.getTextureName(),
                VanityType.fromInt(VanityForTest.TutorShirt.getVanityType()), VanityForTest.TutorShirt.getPrice());
        assertFalse(actualItems.contains(item));
        ArrayList<VanityDTO> newWearing = new ArrayList<>();
        newWearing.add(item);
        gateway.updateCurrentlyWearing(PlayersForTest.JOHN.getPlayerID(), newWearing);
