...
public class DefaultItemsTableDataGatewayRDS implements DefaultItemsTableDataGateway
{
    ...
    public void addDefaultItem(int defaultID, int defaultWearing) throws DatabaseException
    {
        ...
        try
        {
            PreparedStatement stmt = connection.prepareStatement("INSERT INTO DefaultItems SET defaultID = ?, defaultWearing = 0");
    ...
    public VanityDTO getDefaultItem(VanityType type) throws DatabaseException
    {
        ...
        if (toReturn == null)
        {
            throw new DatabaseException("No default exists for vanity type: " + type.name());
        }
        return toReturn;
    }
    ...
    public ArrayList<VanityDTO> getDefaultWearing() throws DatabaseException
    {
        ...
        try
        {
            PreparedStatement stmt = connection.prepareStatement("SELECT * FROM DefaultItems WHERE defaultWearing = 1");
            ResultSet result = stmt.executeQuery();
    ...
    public void setDefaultWearing(int vanityID) throws DatabaseException
    {
        ...
        VanityDTO toBeAdded = VanityItemsTableDataGatewayRDS.getSingleton().getVanityItemByID(vanityID);
        ...
        if (items.contains(toBeAdded))
        {
            ...
            try
            {
                if (toBeRemoved != null)
                {
                    PreparedStatement rmvStmt = connection.prepareStatement("UPDATE DefaultItems SET defaultWearing = 0 WHERE defaultID = ?");
                    rmvStmt.setInt(1, toBeRemoved.getID());
                    rmvStmt.executeUpdate();
                }
                PreparedStatement addStmt = connection.prepareStatement("UPDATE DefaultItems SET defaultWearing = 1 WHERE defaultID = ?");
