...
public class NPCQuestionRowDataGatewayRDS implements NPCQuestionRowDataGateway
{
	 ...
	 */
	public NPCQuestionRowDataGatewayRDS(String questionStatement, String answer, LocalDate startDate,
										LocalDate endDate) throws DatabaseException
	{
		...
		try
		{
			...
			try (ResultSet generatedKeys = stmt.getGeneratedKeys())
			{
				...
				{
					throw new SQLException("Creating NPCQuestion failed, although key was generated");
				}
			}
		...
		{
			throw new DatabaseException("Couldn't create a NPC question record for question with ID " + e);
		}
	}
	...
	public void delete() throws DatabaseException
	{
		...
		try
		{
			PreparedStatement stmt =  connection.prepareStatement( "DELETE FROM NPCQuestions WHERE questionID = ? LIMIT 1");
			stmt.setInt(1, this.questionID);
			stmt.executeUpdate();
		}
		catch (SQLException e)
		{
			throw new DatabaseException("There was no npc question with that id to delete", e);
		}
	}
	...
	{
		QuestionDTO dto = new QuestionDTO(questionID, questionStatement, answer, startDate, endDate);
		return dto;
	}

	/**
	 * @see datasource.NPCQuestionRowDataGateway#getQuestionID()
	 */
	@Override
