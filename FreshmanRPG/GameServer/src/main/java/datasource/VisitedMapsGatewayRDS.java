 ...
 *
 */
public class VisitedMapsGatewayRDS implements VisitedMapsGateway
{
	 ...
	 */
	public VisitedMapsGatewayRDS(int playerID) throws DatabaseException
	{
	...
	private void getAllVisitedMapTitles(int playerID) throws DatabaseException
	{
		...
		String sql = "SELECT Server.mapTitle FROM Server INNER JOIN VisitedMaps ON Server.serverID = VisitedMaps.mapID "
		...
		{
			throw new DatabaseException("Could not get all maps visited for player with id: " + playerID, e);
		}
	}
	...
	{
		ServerRowDataGateway server = ServerRowDataGatewayRDS.findPosAndMapNameFromMapTitle(mapTitle);
		//got the mapname
		mapName = server.getMapName();
		ServerRowDataGateway server2 = ServerRowDataGatewayRDS.findServerInfoFromMapName(mapName);
		mapID = server2.getMapID();
	...
	public static void createTable() throws DatabaseException
	{
		...
		String createSql = "CREATE TABLE VisitedMaps ("
