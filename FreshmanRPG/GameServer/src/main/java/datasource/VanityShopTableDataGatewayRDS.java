...
public class VanityShopTableDataGatewayRDS implements VanityShopTableDataGateway
{
    ...
    public static synchronized VanityShopTableDataGateway getSingleton() throws DatabaseException
    {
        if (singleton == null)
        {
            singleton = new VanityShopTableDataGatewayRDS();
        }
        return singleton;
    }
    ...
    public ArrayList<VanityDTO> getVanityShopInventory() throws DatabaseException
    {
        ...
        VanityItemsTableDataGatewayInterface vanityShopGateway = VanityItemsTableDataGatewayRDS.getSingleton();
        try
        {
            ...
            while (result.next())
            {
                ...
                VanityDTO shopItem = new VanityDTO(actualItem.getID(), actualItem.getName(), actualItem.getDescription(), actualItem.getTextureName(), actualItem.getVanityType(), result.getInt("price"));
