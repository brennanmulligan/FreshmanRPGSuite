...
class PlayerState with _$PlayerState {
  ...
  const factory PlayerState({
    @Default(false) showResponse, //@Default(true) providedInvalidInfo,
  ...
  Future<void> createPlayer(
    ...
    CreatePlayerResponse createPlayerResponse = response.when(
      ...
      failure: (failure) => const CreatePlayerResponse(
          success: false, description: "Network Failure"),
    );
    state = state.copyWith(
      //providedInvalidInfo: createPlayerResponse.responseType == -1,
      showResponse: true, createPlayerResponse: createPlayerResponse,
    );
  }
